...
public class B_Ternary_String {
    ...
    int n=st.length();
    int min=Integer.MAX_VALUE,temp_min=0;
    for(int i=0;i<n;i++)
    {
        ...
        temp_min++;
        if(allPresent(arr)){
            int start=i-temp_min+1;
            int temp=Integer.parseInt(c[start]+"");
            while(arr[temp-1]>1){
                arr[temp-1]--;
                temp_min--;
                start++;
                temp=Integer.parseInt(c[start]+"");
            }
    ...
    static boolean allPresent(int[] arr){
      for(int i=0;i<3;i++)
      {
        if(arr[i]==0)
        return false;
