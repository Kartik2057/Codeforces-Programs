...
public class B_Dreamoon_and_WiFi {
    ...
    double res=(factorial(count)/(factorial(count-diff)*factorial(diff)))/Math.pow(2, count);
    if(count==0)
    res=0;
    if(count==0 && diff==0 && diff2==0)
    res=1;
    pr.println(String.format("%.12f", res));
    pr.close();
    }
