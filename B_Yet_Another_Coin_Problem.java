...
public class B_Yet_Another_Coin_Problem {
    ...
    int dp[] = new int[]{0,1,2,1,2,3,1,2,3,2,1,2,2,2,3,1};
    long t=sc.nextLong();
    int max=0;
    first:
    while(t-->0){
    ...
    for(int i=4;i>=0;i--){
        res = Math.min(res,n/arr[i]+dp[n%arr[i]]);
        if(i>0 && n>arr[i]){
            int rem = arr[i]+n%arr[i];
            res = Math.min(res,n/arr[i]-1+rem/arr[i-1]+dp[rem%arr[i-1]]);
        }
    }
