...
public class C_Tree_Cutting {
    ...
    static int cnt;
    static boolean possible(int ans,ArrayList<ArrayList<Integer>> aList,int k){
        cnt = 0;
        dfs(ans,1,-1,aList);
        return cnt>k;
    }
    static int dfs(int size,int curr,int parent,ArrayList<ArrayList<Integer>> aList){
        int sz = 1;
        for(int a:aList.get(curr)){
            if(a!=parent)
            sz+=dfs(size, a, curr, aList);
        }
        if(sz>=size){
            cnt++;
            return 0;
