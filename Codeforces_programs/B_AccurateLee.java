...
public class B_AccurateLee {
    ...
    String st = sc.next();
    String res="";
    if(st.indexOf("10")==-1&&st.indexOf("01")==-1)
    res=st;
    if(st.charAt(0)=='0'&&st.indexOf("01")!=-1)
    res=st.substring(0, 
    st.indexOf("01")+1);
    if(st.indexOf("10")!=-1)
    res+="0";
    if(st.charAt(n-1)=='1'&&st.lastIndexOf("01", n-1)!=-1)
    res+=st.substring(st.lastIndexOf("01", n-1)+1, n);
    pr.println(res);
    }
    pr.close();
    }
